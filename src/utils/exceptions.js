...
export class MissingPropertyError extends BaseError {
  constructor(componentName, property, description) {
    super(
      ...
      `Component ${componentName} requires property '${property}'${
...
export class PropertyTypeError extends BaseError {
  constructor(componentName, property, type, description) {
    super(
      'PropertyTypeError',
      `The type of property '${property}' of component ${componentName} is invalid, it should be '${type}'${
        description ? `, ${description}` : '.'
      }`
    ) // eslint-disable-line
  }
...
export class MissingKeyError extends BaseError {
  constructor(objName, key, obj, description) {
    super(
      'MissingKeyTypeError',
      `Object ${objName} must have Key '${key}', its value is '${obj}'${
        description ? `, ${description}` : '.'
      }`
    ) // eslint-disable-line
  }
...
export class NoKeyError extends BaseError {
  constructor(objName, key, obj, description) {
    super(
      'NoKeyError',
      `Object ${objName} does not have Key '${key}', its value is '${obj}'${
        description ? `, ${description}` : '.'
      }`
    ) // eslint-disable-line
  }
